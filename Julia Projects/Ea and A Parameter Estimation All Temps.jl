...
T1_moles_timestamps = float.(vec(xf["30C"]["A2:A10"]))
...
trial_1_data = TrialPreprocessedData(
    ...
    #moles_timestamps 
    float.(vec(xf["30C"]["A2:A10"])), 
    #moles_respective_temp_idxs
    T1_moles_respective_temp_idxs,
    #acetic_acid_moles
    float.(vec(xf["30C"]["G2:G10"])),
    #ethanol_moles
    float.(vec(xf["30C"]["I2:I10"])),
    #ethyl_acetate_moles
    float.(vec(xf["30C"]["K2:K10"])),
    #water_moles
    float.(vec(xf["30C"]["J2:J10"])),
)


T2_temp_end_idx = xf["40C Temps"]["I3"]
T2_temp_timestamps = float.(vec(xf["40C Temps"]["K2:K$T2_temp_end_idx"]))
T2_moles_timestamps = float.(vec(xf["40C"]["A2:A10"]))
...
trial_2_data = TrialPreprocessedData(
    ...
    #moles_timestamps 
    float.(vec(xf["40C"]["A2:A10"])), 
    #moles_respective_temp_idxs
    T2_moles_respective_temp_idxs,
    #acetic_acid_moles
    float.(vec(xf["40C"]["E2:E10"])),
    #ethanol_moles
    float.(vec(xf["40C"]["G2:G10"])),
    #ethyl_acetate_moles
    float.(vec(xf["40C"]["I2:I10"])),
    #water_moles
    float.(vec(xf["40C"]["H2:H10"])),
)

T3_temp_end_idx = xf["50C Temps"]["I3"]
T3_temp_timestamps = float.(vec(xf["50C Temps"]["K2:K$T3_temp_end_idx"]))
T3_moles_timestamps = float.(vec(xf["50C"]["A2:A10"]))
...
trial_3_data = TrialPreprocessedData(
    ...
    #moles_timestamps 
    float.(vec(xf["50C"]["A2:A13"])), 
    #moles_respective_temp_idxs
    T3_moles_respective_temp_idxs,
    #acetic_acid_moles
    float.(vec(xf["50C"]["G2:G13"])),
    #ethanol_moles
    float.(vec(xf["50C"]["I2:I13"])),
    #ethyl_acetate_moles
    float.(vec(xf["50C"]["K2:K13"])),
    #water_moles
    float.(vec(xf["50C"]["J2:J13"])),
)
...
guess_params = Float64[log(1000.0), 62.0] #remember, kf_A, kf_Ea
loss(guess_params)
...
guess_lb = [log(300.0), 20.0]
guess_ub = [log(1e7), 100.0]
...
var_1_guesses = collect(range(guess_lb[1], guess_ub[1], length=n_var_1_checks))
var_2_guesses = collect(range(guess_lb[2], guess_ub[2], length=n_var_2_checks))

all_guess_pairs = vec([(v1, v2) for v1 in var_1_guesses, v2 in var_2_guesses])

invalid_guesses = []

for (i, (kf_A_guess, kf_Ea_guess)) in enumerate(all_guess_pairs)     
    guess_params = Float64[kf_A_guess, kf_Ea_guess]

    loss_value = loss([kf_A_guess, kf_Ea_guess])

    if loss_value >= 1e9
        push!(invalid_guesses, i)
    end
end

invalid_guesses

#I'm just now realizing that it would be a lot better to input a vector of guess kf_A values and then optimize to find the corresponding 
#Ea and then choose the one with the lowest loss

deleteat!(all_guess_pairs, invalid_guesses)

optimized_variables = []
losses = []

for (kf_A_guess, kf_Ea_guess) in all_guess_pairs     
    guess_params = Float64[kf_A_guess, kf_Ea_guess]
    optprob = Optimization.OptimizationProblem(optf, Float64[kf_A_guess, kf_Ea_guess], lb=lower_bounds, ub=upper_bounds)
    ...
    res = Optimization.solve(
        ...
        OptimizationOptimJL.BFGS(), 
        #BFGS and Fminbox don't work well here and don't return values that are anywhere close to values defined in literature 
        #IPNewton works pretty well
        f_abstol = 1e-4,
        g_abstol = 1e-4,
    )

    push!(optimized_variables, res.u)
    push!(losses, res.objective)
end
