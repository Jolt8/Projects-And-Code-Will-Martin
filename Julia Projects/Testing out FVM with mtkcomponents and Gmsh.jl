...
grid = togrid("C://Users//wille//Desktop//Legacy-Projects-And-Code-Will-Martin//Julia Projects//newNozzle.msh")
...
reference_shape = RefTetrahedron

poly_interp = Lagrange{reference_shape, 1}() #1 = linear elements 2 = quadratic/curved edges

cell_qr = QuadratureRule{reference_shape}(2) #2 represents the number of integration points. Basically higher number = higher accuracy but more computation

facet_qr = FacetQuadratureRule{reference_shape}(2) 
...
@timed @named rod = ODESystem(connections, t, systems=all_systems)

@timed sys = structural_simplify(rod)  

tspan = (0.0, 10.0)

@timed prob = ODEProblem(sys, [], tspan)

@timed sol = solve(prob)