...
struct FVMGeometry{B<:AbstractBC}
...
function FVM_iter_f!(du, u, p, geo::FVMGeometry, phys::PhysicsParams, beta::Float64, n_cells::Int)
    ...
    for conn in geo.connections
        ...
        u_a = u[idx_a]
        u_b = u[idx_b]
        u_a_2 = u[idx_a + 1 * n_cells]
        u_b_2 = u[idx_b + 1 * n_cells]
        ...
        du[idx_a] -= F_1
        du[idx_b] += F_1
        du[idx_a + 1 * n_cells] -= F_2
        du[idx_b + 1 * n_cells] += F_2
    end
...
nl_func = NonlinearFunction(f_closure, jac_prototype = float.(jac_sparsity), colorvec = colors)
