...
function FVM_iter_f!(du, u, p::FVMProblem, t)
    ...
    for conn in p.connections
        ...
        F = numerical_flux(p.boundary_map[idx_a].physics, u_a[1], u_b[1], conn.area, conn.normal, conn.distance)
...
function my_bc_mapper(grid, cell_id)
    if cell_id in copper_cell_set_idxs
        return HeatBC(:Neumann, 500.0, heated_copper_physics) #use :Dirichlet to fix temperature to initial in HeatBC
    elseif cell_id in steel_cell_set_idxs
        return HeatBC(:Neumann, 300.0, steel_physics)
    end
end
...
for i in 1:n_cells
    u0[1, i] = fvm_prob.boundary_map[i].initial
end
