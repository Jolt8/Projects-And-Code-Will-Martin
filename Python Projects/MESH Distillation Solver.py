...
"""
def short_cut_distliation(feed_infile, feed_fluid, outlet_streams):
    #outlet stream will be a list that looks like this [[new_name, [H2O:0.1, N2:0.9], margin_for_error],[new_name, H2O:0.9, N2:0.1, margin for error],[new_name, H2O:0.5, N2:0.5, margin for error]]
    fluids = ct.Solution(feed_infile)
    saturation_pressures = []
    #find the different saturation pressures
    for i in range(len(outlet_streams)):
        fluids.TPX = 300, 100000, outlet_streams[i][1]
        saturation_pressures.append(fluids.P_sat)
    
    heavy_key_p_sat = min(saturation_pressures)
    light_key_p_sat = max(saturation_pressures)
        
    volatility_ratios = []
    for i in range(len(outlet_streams)):
        volatility_ratios.append(saturation_pressures[i] / heavy_key_p_sat)
    volatility_ratios = [x for _, x in sorted(zip(saturation_pressures, volatility_ratios))]
    saturation_pressures = [x for _, x in sorted(zip(saturation_pressures, saturation_pressures))]
        
    average_volatility = sum(volatility_ratios) / len(volatility_ratios)
    
    bottoms_heavy_key_composition = 
    ...
    print("E")
print(short_cut_distliation([[]]))
"""
"""
fluid = ct.Solution("gri30.yaml")
fluid.TPX = 300, 100000, "H2O:1, H2:1, CO2:1, CH4:1"
#test = fluid.species
#print(test.composition)
print(fluid.concentrations)
"""

"""
def short_cut_distliation_easy(feed_solutions, respective_mole_fractions_in_feed, fractional_recovery_LK_in_distillate, fractional_recovery_HK_in_bottoms):
    #outlet stream will be a list that looks like this [[new_name, [H2O:0.1, N2:0.9], margin_for_error],[new_name, H2O:0.9, N2:0.1, margin for error],[new_name, H2O:0.5, N2:0.5, margin for error]]
    
    saturation_pressures = [feed_solutions[i].P_sat for i in range(len(feed_solutions))]
    
    sorted_pairs = sorted(zip(saturation_pressures, feed_solutions))
    saturation_pressures, sorted_fluids = zip(*sorted_pairs)
    saturation_pressures = list(saturation_pressures)  # Convert back to lists if needed
    ...
    A = math.log10((1 - fractional_recovery_HK_in_bottoms) / fractional_recovery_HK_in_bottoms)
    minimum_number_of_stages = ((math.log10((fractional_recovery_LK_in_distillate / (1 - fractional_recovery_LK_in_distillate)) * (fractional_recovery_HK_in_bottoms / (1 - fractional_recovery_HK_in_bottoms))))
                / (math.log10(relative_volatility)))

    #calculate the recovery ratios for all other components
    distillate_recovery_ratios = [(10 ** A) * (ratio ** minimum_number_of_stages) / (1 + (10 ** A) * (ratio ** minimum_number_of_stages)) for ratio in volatility_ratios]
    ...
    def find_underwood_constant(underwood_constant):
        results2 = []
        for i in range(len(volatility_ratios)):
            results2.append((volatility_ratios[i] * respective_mass_flows[i]) / (volatility_ratios[i] - underwood_constant))
        return sum(results2) - (1 - 0.5)
    max_retries1 = 500
    retries1 = 0
    optimized1 = None
    while optimized1 is None and retries1 < max_retries1:
        optimized1 = scipy.optimize.minimize(find_underwood_constant, [1], method="Nelder-Mead")
        retries1 += 1
        
    def underwood_minimum_reflux(minimum_reflux_ratio):
        results1 = []
        for i in range(len(volatility_ratios)):
                results1.append((volatility_ratios[i] * respective_mass_flows[i]) / (volatility_ratios[i] - optimized1.x))
        return sum(results1) - (minimum_reflux_ratio + 1)

    max_retries2 = 500
    retries2 = 0
    optimized2 = None
    while optimized2.Result is None and retries2 < max_retries2:
            optimized2 = scipy.optimize.minimize(underwood_minimum_reflux, [1], method="Nelder-Mead")
            retries2 += 1
        
    return(saturation_pressures, relative_volatility, volatility_ratios, distillate_recovery_ratios, bottoms_recovery_ratios, minimum_number_of_stages)
"""
...
def distillation_MESH(flasher, feed_stage, feed_temp, feed_pressure, feed_molar_flow, feed_mole_fractions, number_of_stages, stage_heat_exchange, 
    ...
    product_loc_to_idx_map = {loc: i for i, loc in enumerate(product_withdrawal_locations)}
    ...
    def mesh_solver(guess_vars):
        ...
        for j in range(0, N_stages_model):
            ...
            if j in product_loc_to_idx_map:
                prod_idx = product_loc_to_idx_map[j]
                total_prod_flow_at_j = product_molar_flow_rates[prod_idx]
                print("WITHDRAWAL", j)
                if state.phase == 'L':
        ...
        for j in range(0, N_stages_model):
            try:
                ...
                HL_stages.append(liq_state.liquid0.H())
                print("                              HL Success!", "stage:", j, "y_j:", x[j], "T_guess:", T_guess[j], "Column Pressure:", column_pressures[j], "state:", liq_state.phase)
            except:
                print("                              ZONKERS! HL Failiure!", "stage:", j, "y_j:", x[j], "T_guess:", T_guess[j], "Column Pressure:", column_pressures[j], "state:", liq_state.phase)
                
                HL_stages.append(liq_state.H())
            try:
                vap_state = flasher.flash(zs=y[j], T=T_guess[j], P=column_pressures[j])
                HV_stages.append(vap_state.gas.H()) 
    ...
    current_idx = 0
    # Temperature bounds
    lb[current_idx : current_idx + N_stages_model] = 50
    ub[current_idx : current_idx + N_stages_model] = 600
...
stage_heat_exchange = ([3000] + stage_adiabatic_specs + [-4000])  # adiabatic stages
print(stage_heat_exchange)

product_withdrawal_locations = [0, 3]

product_molar_flow_rates=[50, 50]
    
reboiler_pressure = 140000  # Pa
condenser_pressure = 100000  # slight pressure drop
